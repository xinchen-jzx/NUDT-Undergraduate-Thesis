...
SRAM存算一体架构可以有效减少数据的重复搬运，具备突破冯诺依曼架构瓶颈的潜力。由于RISC-V指令集基于开源开放、扩展性强等优势，已经逐渐成为构建存算一体芯片的首选。然而，基于RISC-V构建的存算芯片具有异构、碎片化的特点，这就要求开发者面向不同存算架构开发多个版本的应用，开发效率低、部署难。因此，如何实现将软件操作（包括计算、数据通信等）和硬件配置（如异构计算单元、存储层次等）解耦，以便AI应用开发不再依赖存算IP设计，是解决“编程墙”的关键问题。

不仅如此，AI应用的不规则的发展趋势和存算芯片的异构化、碎片化的现状，使得我们需要探索新的动态编译优化方法，该优化方法既需要能够充分的考虑到AI应用的动态变化的特质，又需要能够充分的挖掘未来存算芯片的架构特征。

故，本课题将面向RISC-V存算芯片修改LLVM编译器，实现对存算指令的支持。通过深入研究LLVM编译器的架构和工作原理，分析RISC-V存算一体芯片的特性，探索如何在LLVM中添加对RISC-V存算模拟器的支持。这包括但不限于对指令集的扩展、内存模型的适配以及优化策略的调整等，以实现应用算子的自动映射和正确指令流的生成，从而更好地协调计算部件，挖掘芯片内部的计算并行性，为RISC-V存算一体芯片提供有力的编译支持。
...
- Triton主要用于加速深度学习应用在GPU上执行效率的编译器，于2021年由OpenAI发布。Triton是一种Python DSL，用于编写机器学习内核，支持多种硬件，包括CPU、GPU和ASIC等等，其能够生成针对特定硬件优化的内核。在中间优化层级，Triton编译器通过块级数据流分析技术，自动优化深度学习模型的执行过程。但是其主要面向英伟达和AMD GPU加速器，对RISC-V存算一体加速器支持有限。
...
- XLA是谷歌于2017年开始开发的一种深度学习领域编译器。其接受来自PyTorch、TensorFlow和JAX等ML框架的模型，在中间优化层级，XLA包括整体模型优化，如简化代数表达式、优化内存数据布局和改进调度等等。但是XLA主要针对TensorFlow优化，对其他框架的支持可能需要额外的工作；同时，其主要面向GPU和谷歌的TPU，其中间表示为深度学习算子级别的抽象，使其难以拓展到RISC-V存算一体加速器。

#figure(
  image("./assets/XLA.png", width: 40%),
  caption: [
    XLA架构图
  ],
) <XLA>

- IREE是一个开源的通用编译和运行时框架，由谷歌发布于2019年。通过输入高层次的机器学习模型，IREE为各种硬件生成优化的可执行代码。在中间优化层级，IREE利用MLIR进行多阶段优化，确保模型在目标平台上高效运行。IREE提供了高性能的编译器后端，硬件抽象层允许轻松添加对新硬件的支持，但是IREE框架主要针对深度学习模型进行端到端的优化，而缺少统一编程模型。
...
- AKG是一个由华为主导开发的深度学习编译器框架。AKG可以接收来自ML框架的模型，生成针对特定硬件优化的内核。在中间优化层级，AKG通过自动性能调优工具，自动生成优化的内核。AKG提供了自动化的调优过程，可以显著提高性能，但是其主要支持华为的昇腾系列AI加速器和英伟达的GPU，对RISC-V存算一体异构芯片支持并不友好。
...
该项目的的理论意义在于探索如何通过解耦软件操作和硬件配置来提高人工智能应用的开发效率和部署便捷性。同时，通过深入研究LLVM编译器和RISC-V存算芯片特性，探索新的动态编译优化方法，以适应人工智能应用动态变化和未来存算芯片架构特征的需求，更好地实现应用算子的自动映射和正确指令流生成，从而协调计算部件，挖掘芯片内部计算并行性。
...
该项目面向RISC-V存算一体芯片修改LLVM编译器，并将其和底层RISC-V指令映射，以便在计算、计算部件协同、通信、存储互联、数据类型等多个角度对硬件特征进行抽象。 
...
+ 资源与工具支持
  ...
  - 现有模拟器与工具链：市面上已有支持RISC-V的模拟器和调试工具，可为项目的测试与验证提供便利。
+ 项目可分阶段进行
...
异构编程语言上手难度较高，缺少实践开发经验。后续将在老师的指导下，以项目为驱动，学习相关异构编程。
