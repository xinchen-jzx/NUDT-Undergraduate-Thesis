...
故，本课题将面向 RISC-V 存算芯片修改 LLVM 编译器，实现对存算指令的支持。通过深入研究 LLVM 编译器架构和工作原理，分析 RISC-V 存算一体芯片的特性，探索如何在 LLVM 中添加对 RISC-V CIM 的支持。这包括但不限于对指令集的扩展、内存模型的适配以及优化策略的调整等，以实现应用算子的自动映射和正确指令流的生成，从而更好地协调计算部件，挖掘芯片内部的计算并行性，为RISC-V存算一体芯片提供有力的编译支持。
...
+ LLVM编译器架构分析与扩展
  ...
  - 后端开发：在LLVM的RISC-V后端中集成新增的存算指令。实现指令的生成、调度和优化，包括在汇编生成和机器码生成阶段的适配。
