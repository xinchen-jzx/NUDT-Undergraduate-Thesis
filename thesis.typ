...
人工智能正在日益渗透到所有的技术领域，而深度学习（Deep Learning）是目前人工智能中最活跃的分支。最近几年，深度学习在计算机视觉 CV@CV17 、自然语言处理 NLP@NLP00 、语音识别 Audio@Audio23 等领域取得了显著的成果。特别是，随着卷积神经网络 CNN@CNN15 、循环神经网络 RNN@RNN20 、图神经网络 GNN@GNN09 和生成对抗网络GAN@GAN14 等新型 AI 模型结构的不断涌现，简化各种深度学习模型的编程对于实现其更广泛的应用至关重要。

与此同时，随着人工智能算法复杂度呈指数级增长，物联网终端设备产生的数据量突破 ZB 量级，传统的计算架构正面临前所未有的“双重困境”：一方面，由于冯诺依曼架构中存储单元与计算单元物理分离，在数据处理过程中，处理器与存储器之间需要不断地通过数据总线来交换数据。处理器性能以每 2 年 3.1 倍的速度增长，而内存性能以每 2 年 1.4 倍的速度增长，导致存储器的数据访问速度越来越跟不上处理器的数据处理速度。处理器的性能与效率因此受到严重制约，从而出现了“存储墙”；另一方面，在冯诺依曼架构下，数据在处理过程中需要不断地从存储器单元“读”数据到处理器单元中，处理完之后再将结果“写”回存储器单元。如此以来，数据在片外存储与运算核心之间的频繁往返迁移，引发了严重的传输功耗问题。根据英特尔的研究显示，在半导体工艺迈入 7nm 时代时，数据搬运功耗高达 35pJ/bit，占比达到 63.7%。数据传输所导致的功耗损失越来越成为芯片发展的制约因素。在AI计算平台上，面对海量的数据，“存储墙”和“功耗墙”的问题愈发用凸显，成为整个计算平台的掣肘。
...
随着深度学习技术在多领域的广泛应用，为其定制硬件加速器已成为了学术界与工业界的研究热点，目前，深度学习加速器主要沿着两大方向发展：

一方面，基于传统计算架构的优化，通过提升硬件性能实现加速效果。例如，寒武纪在 2014 至 2016 年间推出的 DIANNAO 系列论文@DianNao16 ，提出全定制 AI 加速器的设计方案，通过优化数据流和存储结构显著提升深度学习算法的执行速度，为后续研究奠定理论基础；Google 于 2016 年推出的 TPU@TPU16 ，以脉动阵列为核心加速矩阵运算；Yu-Hsin Chen 团队设计的 Eyeriss@Eyeriss17 @Eyerissv219 加速器，通过行固定（Row Stationary，RS）等方法降低数据搬运延迟和能耗开销；清华 Thinker 团队提出的基于 CGRA 的可重构加速器@CGRA18 ，可动态配置计算引擎单元阵列以支持多种神经网络运算。

另一方面，颠覆传统冯诺依曼架构的创新方向正崭露头角。2010 年惠普实验室利用忆阻器实现简单布尔逻辑功能@Memristive10 ；2016 年，加州大学圣塔芭芭拉分校谢源教授团队提出的 PRIME 架构，首次验证了基于浮栅晶体管的存内计算在深度学习中的效用，相较于传统方案实现功耗降低约 20 倍、速度提升约 50 倍，引发产业界广泛关注@PRIME16 。

这两种技术路线相互补充，共同推动着深度学习硬件加速领域的快速发展，为人工智能技术的进一步突破提供了坚实基础。
...
第 4 章为智能识别 NPU 加速指令。本章着重介绍了编译器如何在 LLVM IR 中间表示上进行应用特征分析，识别出可加速的 LLVM IR 范式。着重分析了三种典型的可加速范式，通过对这几类加速范式的循环结构深入分析，为编译器智能识别出 NPU 加速指令提供关键依据。

第 5 章为指令调度。本章围绕如何在 CPU 和 NPU 异构计算单元之间进行高效指令调度展开了详细的介绍。
...
深度学习编译器的工作是接收上层深度学习模型作为输入，运用各种优化技术，生成硬件平台执行所需要的指令，确保神经网络模型在硬件上高效执行，其处于深度学习框架和底层硬件之间，它提供了一种中间层，可以覆盖不同的加速器硬件，从而为深度学习模型的部署与运行提供强大的支持。其大致的框架图如@DLCompiler 所示。
...
本文基于 RISC-V 存算一体模拟器的深度学习编译器数据流具体如@Overview 所示。鉴于 LLVM 优秀的模块化与可扩展性设计，以及其具有足够表现力的中间表示 —— LLVM IR，本文编译器采用 LLVM IR 作为中间表示形式，并将 LLVM 作为后端，来复用其强大的优化器与代码生成器，同时通过扩展 RISC-V 加速指令来实现对 RISC-V 存算一体模拟器的支持。下面基于@Overview 对本文编译器各层次的主要功能及设计思想进行简要概述。
...
ONNX@ONNX （Open Neural Network Exchange，开放神经网络交换），是一种用于表示深度神经网模型的开放格式，由微软和 Facebook 于 2017 年共同推出。作为一个标准化框架，ONNX 确立了一组与运行环境和硬件平台无关的标准格式，旨在实现不同深度学习训练和推理框架间的模型转换与互操作。它不仅定义了一种可扩展的计算图模式、各类运算符，还涵盖了标准化的数据类型，为不同框架提供了一种统一的 IR。如@yolov3-onnx 使用 Netron@Netron 对 yolov3-tiny @yolov3-tiny20 模型进行可视化。
...
ONNX 结构如@onnx-ir 所示，加载 ONNX 模型后，得到的是一个 `ModelProto` 对象，其中不仅包含了版本信息和生产者信息，还涵盖了至关重要的 `GraphProto` 组件。`GraphProto` 内嵌四个关键的可重复数组：node（`NodeProto` 类型）、input（`ValueInfoProto` 类型）、output（`ValueInfoProto` 类型）以及 initializer（`TensorProto` 类型）。其中，node 存储了模型的所有计算节点；input 记录了模型的所有输入节点；output 保存了模型的所有输出节点；initializer 则存放着模型的全部权重数据。每个计算节点内部同样配置了 input 和 output 两个数组，借助 input 与 output 的指向关系，我们能够依据上述信息高效地构建出深度学习模型的拓扑结构图。此外，每个计算节点还内含一个 `AttributeProto` 数组，用于详细描述该节点的属性特征，例如，卷积层（Conv）的属性便包括分组数（`group`）、填充（`pads`）和步幅（`strides`）等关键参数。
...
FASHION-MNIST 是由 Zalando 研究团队提供的图像数据集，用于替代传统的 MNIST 手写数字集。该数据集包含了来自 $10$ 个不同类别的共 $70000$ 张商品正面图片，涵盖了各种类型的服装和配饰。FASHION-MNIST 在数据规模、格式规范以及训练集与测试集的划分比例上，均与原始的 MNIST 数据集保持高度一致，其中训练集包含 $60000$ 张图像，测试集包含 $10000$ 张图像，所有图片均为 $28 times 28$ 像素的灰度图像。

在本次实验中，我们采用了一个自定义设计的 FASHION-MNIST 网络模型进行研究，该模型专门针对 FASHION-MNIST 数据集进行了训练优化。网络模型的具体结构如图@FASHION-MNIST 所示，其对应的 ONNX 可视化结果则展示在图@ONNX-Model 中。该网络架构由 $3$ 个卷积层和 $2$ 个全连接层组成，在除最后一个全连接层之外的每个层的输出端，均采用了 ReLU 激活函数来进行非线性激活处理，从而增强模型对复杂特征的学习能力。
