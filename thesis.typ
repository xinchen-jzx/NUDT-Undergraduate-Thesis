...
深度学习作为机器学习领域中一个非常重要的分支，近几年来发展迅速，在计算机视觉 @CV17 、自然语言处理 @NLP00 、电子商务 @e-commerce16 和药物研发 @drug18 等领域都取得了显著的成果。随着卷积神经网络（CNN） @CNN15 、循环神经网络（RNN） @RNN20 、长短期记忆网络（LSTM） @LSTM97 和生成对抗网络（GAN） @GAN14 等多种深度学习模型的不断涌现，简化各种深度学习模型的编程对于实现其更广泛的应用至关重要。

与此同时，随着人工智能算法复杂度呈指数级增长，以及物联网终端设备产生的数据量也突破 ZB 量级，传统的计算架构正面临前所未有的“双重困境”：一方面，受限于冯诺依曼架构中存储单元与计算单元的物理分离特性，在数据处理过程中，处理器与存储器之间需要不断地通过数据总线交换数据。处理器性能以每 2 年 3.1 倍的速度增长，而内存性能以每 2 年 1.4 倍的速度提升，导致存储器的数据访问速度越来越跟不上处理器的数据处理速度。处理器的性能与效率因此受到严重制约，从而出现了“存储墙”；另一方面，在冯诺依曼架构中，数据在处理过程中需要不断地从存储器单元“读”数据到处理器单元中，处理完之后再将结果“写”回存储器单元。数据在片外存储与运算核心之间的频繁迁移带来了严重的传输功耗问题。根据英特尔的研究显示，在半导体工艺进入 7nm 时代时，数据搬运功耗高达 35pJ/bit，占比达到 63.7%。数据传输所导致的功耗损失越来越成为芯片发展的制约因素。在AI计算平台上，面对海量的数据，“存储墙”和“功耗墙”的问题愈发用凸显，成为整个计算平台的掣肘。
...
基于上述现状与需求，本课题将面向 RISC-V SRAM 存算一体芯片修改 LLVM 编译器，实现对存算指令的支持，这包括但不限于对指令集的扩展、内存模型的适配、NPU 指令的智能识别、指令调度以及优化策略的调整等。具体而言，编译器对应用程序进行应用特征分析，识别出可以加速的计算部分，并转为特定的 RISC-V 加速指令，以自动将它们卸载到 CIM 加速器的 NPU 核心进行计算计算，同时运行时在 CPU 和 NPU 异构计算单元之间进行指令的动态调度，充分利用 RISC-V 通用核心、NPU 加速核心的高效能特征，为 RISC-V 存算一体芯片构建起坚实可靠的编译支持体系，助力其在实际应用中发挥出卓越的性能表现。
...
一方面，沿用传统的计算架构来提高硬件的加速性能，如 GPU、AISC、FPGA 等。寒武纪在 2014 年到 2016 年间陆续发表了 DIANNAO 系列论文@DianNao16 ，提出了一系列全定制 AI 加速器的设计方案，通过优化数据流和存储结构，显著提升了深度学习算法的执行速度，为后续研究奠定了重要的理论基础和设计思路；Google 于 2016 年推出以脉动阵列为计算核心加速矩阵运算的 AI 加速器 TPU@TPU16 ；同时 Yu-Hsin Chen 等人针对缓存与内存之间大量数据搬移问题设计了一种具有可重配置功能的深度学习加速器 Eyeriss@Eyeriss17 ，主要通过行固定（Row Stationary，RS）等方法来降低数据搬运带来的延迟和能耗开销，之后又提出了一种用于紧凑神经网络模型的加速器 Eyeriss v2@Eyerissv219 ；清华的 thinker 团队则提出了一种基于 CGRA 的可重构加速器@CGRA18 ，该加速器可以通过对计算引擎单元阵列进行动态配置，实现以相同的硬件支持包括卷积在内的大多数神经网络运算。
...
本文的主要工作是面向 RISC-V SRAM 存算一体芯片修改 LLVM 编译器，实现对存算指令的支持。编译器对应用程序进行应用特征分析，识别出可以加速的计算部分，并转为特定的 RISC-V 加速指令，充分利用 RISC-V 已有的指令集实现在执行 AI 任务时对各类计算资源的灵活调度，充分发挥 SRAM 存算一体阵列高能效、高算力密度的硬件优势。本文的主要研究工作包括以下几个方面：
...
全文共包含 6 个章节，主要研究结构安排如@Structure 所示：
...
第二章为主要技术基础。介绍了本文研究所涉及的相关基础概念，首先介绍了 RISC-V 基础指令集和扩展指令集；接着对存算一体加速器的基本概念进行了介绍；最后还阐述了 LLVM 编译器的结构和 LLVM IR 的设计。

第三章为基于 RISC-V 存算一体加速器的编译器总体描述。给出了本文编译器的总体结构，对本文编译器从编译器前端、中端以及后端各个模块的设计进行了具体介绍。
...
存算一体（Computing in Memory，CIM）的概念最早可以追溯到 20 世纪 70 年代，美国斯坦福研究所（Stanford Institute）的 Kautz 首次提出了存算一体的概念，其核心思想是直接利用内存完成计算功能，减少数据在存储器和处理器之间的搬运开销。

目前存算技术在按照以下路线在演进：

- 查存计算（Processing With Meomory）：GPU 中对于复杂函数就采用了这种计算方法，最早已落地多年的技术。通过在存储芯片内部查表来完成计算操作。

- 近存计算（Computing Near Memory）：典型代表是 AMD 的 Zen 系列 CPU，技术方案已经比较成熟。计算操作由位于存储区域外部的独立计算芯片/模块完成。这种架构设计的代际设计成本较低，适合传统架构芯片转入。将 HBM 内存（包括三星的 HBM-PIM）与计算模组（裸 Die）封装在一起的芯片也属于这一类。

- 存内计算（Computing In Memory）：典型代表是 Mythic、闪忆、知存、九天睿芯等。计算操作由位于存储芯片/区域内部的独立计算单元完成，存储和计算可以是模拟的也可以是数字的。这种路线一般用于算法固定的场景算法计算。

- 存内逻辑（Logic In Memory）：这是较新的存算架构，典型代表包括 TSMC 和千芯科技。这种架构数据传输路径最短，同时能满足大模型的计算精度要求。通过在内部存储中添加计算逻辑，直接在内部存储执行数据计算。
...
#h(2em) 这些原则和最佳实践使 LLVM IR 成为一个强大且灵活的工具，用于编译器开发和代码优化。它的模块化设计、强类型系统、丰富的指令集和目标无关性使其适用于广泛的应用场景，从语言前端到高级优化和代码生成。

== 本章小结

本章主要描述了论文所涉及的相关技术基础，首先介绍了 RISC-V 基础指令集和扩展指令集；接着对存算一体加速器的基本概念和发展进行了基本介绍；最后本章对 LLVM 编译器结构和其对应的 IR 设计进行了介绍。


#pagebreak()

= 基于 RISC-V 存算一体加速器的编译器
...
在实际执行调度时，对于同等优先级的指令，由于具体调度方案的不同，会出现不同的情况，可以通过添加其他度量标准进一步优化优先级计算方案。尽管表调度方法不能保证得到最优调度结果，但它是接近最优解的。
...
系统架构图如@Runtime 所示，更直观地呈现了指令动态调度过程中各组成部分之间的关系以及数据流向。
