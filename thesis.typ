...
基于上述现状与需求，本课题将聚焦于面向 RISC-V 存算一体芯片的编译支持工作，构建一个可以面向 RISC-V SRAM 存算一体芯片进行自动后端优化的编译器系统，使其能够有效支持存算指令的执行与处理。具体而言，编译器对应用程序进行应用特征分析，识别出可以加速的计算部分，并转为特定的 RISC-V 加速指令，以自动将它们卸载到 CIM 加速器的 NPU 核心进行计算计算，同时运行时在 CPU 和 NPU 异构计算单元之间进行指令的动态调度，充分利用 RISC-V 通用核心、NPU 加速核心的高效能特征，为 RISC-V 存算一体芯片构建起坚实可靠的编译支持体系，助力其在实际应用中发挥出卓越的性能表现。
...
充分利用RISC-V已有指令集实现在执行AI任务运行时对各类计算资源的灵活调度，充分发挥SRAM存算一体阵列高能效、高算力密度的硬件优势。
...
本章主要介绍了如何在 CPU 和 NPU 异构计算单元之间进行指令的静态和动态调度。当编译器后端智能识别出 NPU 加速指令后，如何在异构架构下调度生成的 RISC-V 指令来充分发挥 SRAM 存算一体阵列高能效、高算力密度的硬件优势。
...
2. 进一步优化编译器的性能。目前虽然在算子融合、指令识别与卸载等方面取得了一定成果，但仍可探索更高效的算法和策略，以获取更高的计算效率和更低的内存占用，例如引入机器学习算法智能来地决策算子融合的顺序和程度等。