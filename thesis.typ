...
深度学习编译器的工作是将上层深度学习模型通过各种优化技术生成硬件平台执行所需要的指令，确保神经网络模型在硬件上高效执行。其大致的框架图如@DLCompiler 所示。
...
（1）编译器前端。前端的主要作用为负责接收和处理来自不同 AI 框架的模型，并将其转换为 LLVM IR，进行初步优化。目前本文编译器仅支持一种模型载入方式，即借助解析器载入一个完整的模型文件，接收的模型文件格式为开放神经网络交换格式 ONNX。选定 ONNX 作为编译器输入格式的主要原因为，ONNX 的设计定位就是作为不同框架间或框架与工具间模型转换的中间格式，目前包括 PyTorch 和 TensorFlow 在内的大多数主流深度学习框架都支持将模型导出或转为 ONNX 格式，所以使用 ONNX 作为输入的优势为无需额外添加多个模型解析器，便能够直接或间接的支持多种框架模型。
...
在 ONNX 中，顶层结构是一个"Model"，用于将 MeraData 与 Graph 关联起来。ONNX 中的运算符分为一组原语和函数，其中函数是可以通过其他运算符的子图来表达计算结果的运算符。Graph 用于描述函数。Graph 中包含 Node List、Input List、Output List和初始化器（输入的常量值或默认值）的列表。无环数据流图构建为图形中节点列表的拓扑排序。Graph 中的每个 Node 都包含它调用的运算符的 Name、Input List、Output List以及与运算符关联的Attribute List。输入和输出可以标记为可变参数或可选参数。

#figure(
  image("./images/onnx-ir.png", width: 100%),
  caption: [
    ONNX IR 结构
  ],
) <onnx-ir>
...
编译器中间优化器是对计算图进行与硬件结构无关的优化，在不影响计算结果的情况下降低模型的空间复杂度以及计算复杂度从而减少模型在加速器上的推理时间。通过遍历计算图节点并执行图转换来应用这些优化。优化 passes 可以捕获计算图的特定特征，并对其进行重写以实现优化。中间层图优化策略中根据粒度主要分为层级优化、张量级优化和元素级优化。基于 RISC-V 存算一体加速器，本文编译器从层级优化出发，使用算子融合来提高神经网络模型执行效率，这可以减少中间结果的内存访问，从而减少片上内存和片外内存之间的数据传输，有助于提高带宽利用率。
