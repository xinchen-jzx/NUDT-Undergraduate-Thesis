...
#figure(
  ...
  caption: [
    (a)模拟式存内计算基本结构；(b)数字式存内计算基本结构@Intro24
  ],
)
...
全文共包含 6 个章节，主要研究结构安排如@fig:Structure 所示：
...
RISC-V 采用“Base ISA + Extensions”的模块化设计 —— 基础指令集（如 RV32I、RV64I 等）提供最小的通用计算能力，利用标准扩展（M、A、F、D、C、B、V 等）或自定义扩展（Custom-0/1/2/3）满足特定需求。其中，标准扩展由 RISC-V 国际基金会维护，保障跨平台兼容；而自定义扩展则面向专用应用，设计者可在保留的编码空间内自由定义指令，实现硬件加速功能。如@RV-ISA 所示。
...
如@CIM-Structure ，在本文使用的 RISC-V 存算一体模拟器中，NPU Core 作为 NPU 的一部分，与外部的 RISC-V Core 共享程序计数器（PC），其二者通过 NICE 接口来进行通信，RISC-V Core 与 NPU Core 通过扩展指令或内存映射的方式互连，共同组成 NPU。本文使用的 CIM 内核由四大模块构成：  
...
#figure(
  image("./plot/npu_work.png", width: 70%),
  caption: [
    NPU 工作占比
  ],
) <NPU-work>
...
本次实验不仅基于 FASHION MNIST 数据集完成了自定义网络模型的推理任务，还选取了神经网络中比较常见的 $20$ 种算子在 RISC-V 存算一体模拟器中来评估 NPU 核心的性能表现，性能测试结果如@result 所示，深度学习常见的算子的分类如@operator 所示。
